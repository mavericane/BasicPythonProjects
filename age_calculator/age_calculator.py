...
while True:
    try:
        ...
        if today.year >= birthyear >= 0:
            ...
            print(termcolor.colored("Your date of birth cannot be in the future.", "yellow"))
    except:
...
while True:
    try:
        birthmonth = int(input("Please enter the month you were born(Number of month): "))
...
while True:
    try:
        birthday = int(input(f"Please enter the date you were born days between: 1-{days}: "))
        if days >= birthday >= 1:
            break
...
#Age output
print(termcolor.colored(f"You have lived for: {lived_years} years, {lived_months} months and {lived_days} days", "green"))
...
if leap_counter > 0:
    while True:
        consider_leap = input(termcolor.colored("Do you want to consider leap years and extra 1 day(s) in your age? (Y yes, N no): ", "cyan"))
        if consider_leap.casefold() == "y" or consider_leap.casefold() == "yes":
            ...
            if lived_days+leap_counter >= 31:
                ...
                print(termcolor.colored(f"You have lived for {lived_years} years, {lived_months} months and {lived_days_with_leap} days.", "green"))
            else:
                print(termcolor.colored(f"You have lived for {lived_years} years, {lived_months} months and {lived_days+leap_counter} days.", "green"))
            print(termcolor.colored(f"You have lived for: {age_in_days+leap_counter} days", "green"))
            break
    ...
    print(termcolor.colored("You did not live in a leap year (probably you are less than 4 years old xD)", "green"))