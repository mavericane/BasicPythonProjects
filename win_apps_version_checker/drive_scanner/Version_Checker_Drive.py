...
def get_version_number(file_path):
    ...
    version = [
        str(win32api.HIWORD(ms_file_version)),
        str(win32api.LOWORD(ms_file_version)),
        str(win32api.HIWORD(ls_file_version)),
        str(win32api.LOWORD(ls_file_version)),
    ]
...
# Welcome output
print(
    """Welcome to the Version Checker Drive program
...
for app in apps:
    while True:
        result = input(
            f"Do you want to check for \"{termcolor.colored(app[0], 'yellow')}\":"
        )
...
print(
    termcolor.colored(
        "Your selected softwares will be checked in few moments", "yellow"
    )
)
